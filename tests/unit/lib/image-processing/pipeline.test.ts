...
describe('processImage', () => {
  ...
  it('should produce valid jsonb structure', async () => {
    ...
    for (const [key, paths] of Object.entries(result.variantsJsonb)) {
      expect(paths.avif).toContain(`-${(PHOTO_VARIANTS as Record<string, { fileSuffix: string }>)[key]!.fileSuffix}.avif`)
      expect(paths.webp).toContain(`-${(PHOTO_VARIANTS as Record<string, { fileSuffix: string }>)[key]!.fileSuffix}.webp`)
    }
  })
})
